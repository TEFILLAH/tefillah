...
if JWT_SECRET == _jwt_default:
    ...
    if os.environ.get('RAILWAY_ENVIRONMENT') or os.environ.get('PRODUCTION'):
        raise RuntimeError("FATAL: JWT_SECRET must be set to a strong random value in production!")
    logger.warning("⚠️  Using default JWT_SECRET — set JWT_SECRET env var before deploying to production!")
JWT_ALGORITHM = "HS256"
...
SENDER_EMAIL = os.environ.get('SENDER_EMAIL', 'support@tefillah.in')
...
# Create the main app
app = FastAPI(title="TEFILAH API", description="Sacred Prayer Request Platform - Production Ready")
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request, call_next):
        ...
        response.headers["Content-Security-Policy"] = "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; connect-src 'self' https://*.tefillah.in https://*.railway.app; font-src 'self'; frame-ancestors 'none'"
        return response
...
async def generate_bible_verse(request: Request, language: str = "en"):
    """Generate a random Bible verse using LLM"""

    client_ip = get_client_ip(request)
    if not check_rate_limit(f"verse:{client_ip}", 10):
        raise HTTPException(status_code=429, detail="Too many requests. Please try again later.")

    import random
    themes = ["prayer", "faith", "hope", "peace", "comfort", "strength", "trust", "healing", "gratitude", "love", "patience", "courage", "forgiveness", "joy", "wisdom"]
    books_hint = ["Psalms", "Proverbs", "Isaiah", "Matthew", "Romans", "Philippians", "Hebrews", "James", "1 Peter", "John", "Ephesians", "Colossians", "Jeremiah", "Lamentations", "2 Corinthians"]
    chosen_theme = random.choice(themes)
    chosen_book_hint = random.choice(books_hint)
...
Provide an inspiring Bible verse about {chosen_theme}. Try to pick a verse from {chosen_book_hint} if possible, but any book of the Bible is fine.
...
{{
    ...
    llm_response, error = await generate_llm_response(f"Provide a Bible verse about {chosen_theme}.", system_prompt)

    if error:
        logger.error(f"Verse generation LLM error: {error}")

    if llm_response:
        logger.info(f"Verse LLM raw response (first 300 chars): {llm_response[:300]}")
        parsed = parse_llm_json(llm_response)
        if parsed and parsed.get("verse"):
            return BibleVerseResponse(
                verse=parsed.get("verse"),
                reference=parsed.get("reference", "Unknown"),
                theme=parsed.get("theme", "prayer")
            )
        else:
            logger.error(f"Failed to parse verse LLM JSON response")
    ...
    return BibleVerseResponse(**selected)
...
async def forgot_password(data: ForgotPasswordRequest, request: Request, background_tasks: BackgroundTasks):
    ...
    # Check all collections (users, partners, admins)
    user = await db.users.find_one({"email": email})
    collection = db.users
    if not user:
        user = await db.partners.find_one({"email": email})
        collection = db.partners
    if not user:
    ...
    # Check for social-auth-only accounts (no password set)
    if user.get("auth_provider") and user.get("auth_provider") not in ("email", "unknown") and not user.get("password_hash"):
        return {"message": "If an account with this email exists, a reset code has been sent."}
...
async def verify_firebase_token(token: str) -> Optional[dict]:
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as http_client:
            ...
            if response.status_code == 200:
                data = response.json()
                expected_client_id = os.environ.get("GOOGLE_WEB_CLIENT_ID", "")
                if expected_client_id and data.get("aud") == expected_client_id:
                ...
                elif not expected_client_id:
                    # No client ID configured — accept but log warning
                    logger.warning("GOOGLE_WEB_CLIENT_ID not set, accepting Google token without audience check")
                    return {
                        "uid": data.get("sub"),
                        "email": data.get("email", ""),
                        "name": data.get("name", ""),
                    ...
                    logger.warning(f"Google token audience mismatch: expected={expected_client_id}, got={data.get('aud')}")
    except Exception as e:
        logger.warning(f"Google tokeninfo verification failed: {e}")

    # Method 3: Fallback - decode JWT payload (works offline but doesn't verify signature)
    try:
        parts = token.split('.')
        if len(parts) != 3:
            return None
        ...
        # Determine provider - supports Firebase tokens and direct Google OAuth tokens
        iss = decoded.get('iss', '')
        provider = "unknown"
        if 'accounts.google.com' in iss or decoded.get('aud', '').endswith('.apps.googleusercontent.com'):
            provider = "google.com"
        elif decoded.get("firebase"):
            provider = decoded["firebase"].get("sign_in_provider", "unknown")
...
async def complete_social_auth(data: SocialAuthCompleteRequest, request: Request, background_tasks: BackgroundTasks):
...
async def register_partner(partner_data: PartnerCreate, request: Request, background_tasks: BackgroundTasks):
    ...
    # Check if email exists in users or partners
    existing_user = await db.users.find_one({"email": partner_data.email.lower()})
...
async def login_partner(credentials: PartnerLogin, request: Request):
    ...
    partner = await db.partners.find_one({"email": credentials.email.lower()})
...
async def get_partner_requests(
    ...
    query = {"assigned_partner_id": partner["_id"]}
    if status:
        query["status"] = status
    
    skip = (page - 1) * limit
    cursor = db.prayer_requests.find(query, {"_id": 1, "content": 1, "location_city": 1, "location_country": 1, "category": 1, "status": 1, "submitted_at": 1, "assigned_at": 1})
...
async def mark_prayer_as_prayed(
    ...
    # Enforce 1-hour minimum wait after assignment
    ...
    # Generate LLM comfort message and notify the prayer submitter
    prayer_user_id = prayer.get("user_id")
    if prayer_user_id:
        # Generate a personal comfort message via LLM
        comfort_prompt = f"Prayer request: {prayer.get('content', '')[:300]}"
...
Be warm, personal and hopeful. Do NOT include any JSON formatting — just the plain message text."""
        ...
        comfort_message = "A prayer partner has prayed over your request and will continue to uphold you in prayer. May God's peace and comfort surround you today."
        try:
            llm_result, llm_err = await generate_llm_response(comfort_prompt, comfort_system, prayer_user_id)
            if llm_result and not llm_err:
                comfort_message = llm_result.strip().strip('"').strip("'")
        except Exception as e:
            logger.warning(f"LLM comfort generation failed for prayer notification: {e}")
...
async def create_first_admin(admin_data: AdminCreate, request: Request):
    ...
    if existing_admin:
        raise HTTPException(status_code=400, detail="Admin already exists. Use admin invite flow.")
...
async def update_user(user_id: str, status: Optional[str] = None, is_verified: Optional[bool] = None, admin: dict = Depends(get_current_admin)):
    update_fields = {}
...
async def update_partner(
    ...
    result = await db.partners.update_one({"_id": partner_id}, {"$set": update_fields})
    if result.matched_count == 0:
...
async def assign_prayer_to_partner(
    ...
    # Update prayer with new assignment
    await db.prayer_requests.update_one(
        {"_id": prayer_id},
        {"$set": {
            "assigned_partner_id": partner["_id"],
    ...
    if bulk.action == "delete":
        ...
        for pid in bulk.prayer_ids:
            result = await db.prayer_requests.update_one(
                {"_id": pid, "status": {"$ne": "prayed"}},
                {"$set": {"assigned_partner_id": None, "assigned_partner_name": None, "status": "pending", "assigned_at": None}}
            )
            results["success" if result.matched_count > 0 else "failed"] += 1
...
async def send_email_broadcast(email_req: EmailBroadcast, background_tasks: BackgroundTasks, admin: dict = Depends(get_current_admin)):
    ...
    if email_req.target == "all":
        user_cursor = db.users.find({"email": {"$exists": True}}, {"email": 1, "name": 1})
        async for user in user_cursor:
            recipients.append({"email": user["email"], "name": user.get("name", "")})
        partner_cursor = db.partners.find({"email": {"$exists": True}}, {"email": 1, "name": 1})
        async for partner in partner_cursor:
            recipients.append({"email": partner["email"], "name": partner.get("name", "")})
    elif email_req.target == "users":
        cursor = db.users.find({"email": {"$exists": True}}, {"email": 1, "name": 1})
        async for user in cursor:
            recipients.append({"email": user["email"], "name": user.get("name", "")})
    elif email_req.target == "partners":
        cursor = db.partners.find({"email": {"$exists": True}}, {"email": 1, "name": 1})
        async for partner in cursor:
            recipients.append({"email": partner["email"], "name": partner.get("name", "")})
    elif email_req.target == "specific" and email_req.target_ids:
        for collection in [db.users, db.partners]:
            cursor = collection.find({"_id": {"$in": email_req.target_ids}}, {"email": 1, "name": 1})
            async for doc in cursor:
                recipients.append({"email": doc["email"], "name": doc.get("name", "")})
    else:
        raise HTTPException(status_code=400, detail="Invalid target. Use: all, users, partners, specific")
            ...
            try:
                async with httpx.AsyncClient() as client_http:
                    await client_http.post(
                        ...
                        json={"from": SENDER_EMAIL, "to": recipient["email"], "subject": email_req.subject, "html": html_template}
                    )
                sent += 1
            except Exception as e:
                failed += 1
...
async def seed_database(admin: dict = Depends(get_current_admin)):
    """Seed database with sample prayer cells"""
